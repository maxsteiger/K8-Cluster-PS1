$version = 'v1.0.3'
$workdir = "C:\ClusterStorage\Volume1\K8_Cluster"
...
switch ($cni) {
  'flannel' {
    ...
    $cniyaml = "$workdir\yaml-files\kube-flannel.yml"
...
function New-Machine($zwitch, $vmname, $cpus, $mem, $hdd, $vhdxtmpl, $cblock, $ip, $mac) {
  ...
  $vmiso = "$vmdir\$vmname.iso"

  New-Item -itemtype directory -force -path $vmdir | Out-Null

  # Check if .vdhx exists, otherwise create from vdhx template
  if (!(Test-Path $vhdx)) {
  ...
  # Check if VM exists
  $currVmExists = Get-VM -Name $vmname -ErrorAction SilentlyContinue
  if (!($currVmExists)) {
...
#
$prefix#
$prefix$($cblock).100 master
$prefix$($cblock).101 node1
$prefix$($cblock).102 node2
$prefix$($cblock).103 node3
$prefix$($cblock).104 node4
$prefix$($cblock).105 node5
$prefix$($cblock).106 node6
$prefix$($cblock).107 node7
$prefix$($cblock).108 node8
$prefix$($cblock).109 node9
$prefix$($cblock).110 node10
$prefix#
$prefix#
"@
    }
...
function New-Nodes($num, $cblock) {
  1..$num | ForEach-Object {
    ...
    New-Machine -zwitch $zwitch -vmname "node$_" -cpus 4 -mem 4GB -hdd 40GB `
      -vhdxtmpl $vhdxtmpl -cblock $cblock -ip $(110 + $_)
  }
}
...
"@
  ...
  ^Deploy-Master$ {
    New-Machine -zwitch $zwitch -vmname 'master' -cpus $cpus `
      ...
      -vhdxtmpl $vhdxtmpl -cblock $cidr -ip '100' -mac $macs[0]
  }
